...
# Base strategy
def wighted_base_strategy(df: DataFrame, stake_amount=1000.0, starting_balance=1000.0, stop_loss=0.05) -> dict:
    '''
    En orden las columnas del dataframe:
      High, Low, Open, Close, Volume, Adj Close, very_bullish,
      very_bearish, bullish, bearish, neutral, change_24hrs
    ...
    for date, row in zip(indexes, df.values):
        ...
        if not open_trade:
          # Verificamos si hay señal de compra
          if weight * n_very_bullish + n_bullish > weight * n_very_bearish + n_bearish and balance > 0:
            # buy signal...
            open_trade = True 
            current_trade = register_open_trade(trade_id, balance, stake_amount, close_price, date)
            amount = 0
          ...
          # Con un trade abierto verificamos si hay señal de venta
          if weight * n_very_bullish + n_bullish < weight * n_very_bearish + n_bearish:
              # sell signal...
              open_trade = False
